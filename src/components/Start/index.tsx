...
const Start = () => {
  ...
  useEffect(() => {
    ...
    if (
      ...
      messageLogContext.asyncMessages[messageLogContext.asyncMessages.length - 1].includes(
        "Closing Python process"
      )
    ) {
      handleStop();
    }
  ...
  useEffect(() => {
    if (!firstTimeSetup) {
      try {
        ...
        localSettings.nightmare.nightmareSummonElements = fetchSummonElements(
          localSettings.nightmare.nightmareSummons
        );

        // Stringify the contents and prepare for writing to the specified file.
        const jsonString = JSON.stringify(localSettings, null, 4);
        const settingsFile: FsTextFileOption = {
  ...
  useEffect(() => {
    if (
      messageLogContext.messageLog.find(message =>
        message.includes("Child process finished with code")
      ) !== undefined
    ) {
  ...
  const handleReady = () => {
    ...
    if (
      ...
      botStateContext.setReadyStatus(
        botStateContext.settings.game.item !== "" &&
          botStateContext.settings.game.mission !== "" &&
          botStateContext.settings.game.summons.length !== 0
      );
    } else if (
      ...
      botStateContext.setReadyStatus(
        botStateContext.settings.game.item !== "" && botStateContext.settings.game.mission !== ""
      );
    } else {
      botStateContext.setReadyStatus(
        farmingMode === "Generic" &&
